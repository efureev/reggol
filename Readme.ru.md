...
- Опциональное место вызова (`WithCaller`), по-прежнему без аллокаций.
- 256 цветов и 24-битный вывод с автоматическим понижением под возможности терминала.
- **Ни одной зависимости** — включая косвенные.
...
`WithCaller` записывает, откуда пришла запись:
...
Путь сокращается до последних двух сегментов: так виден пакет, но в логи не
утекает раскладка каталогов сборочной машины. Text и JSON кладут его в поле
`caller`.

По умолчанию выключено, потому что это не бесплатно: захват и резолв счётчика
команд стоят примерно в 2.5 раза дороже самой записи — около 78 нс против 200 нс
на M5 Pro, — хотя аллокаций по-прежнему ноль. Если важны лишь отдельные места,
включайте точечно через `Event.Caller()`.

Код, который оборачивает reggol собственными хелперами, обязан учесть добавленные
кадры, иначе в логе окажется сам хелпер:

```go
func (l MyLogger) Info(msg string) { l.inner.AddCallerSkip(1).Info().Msg(msg) }
```

`EventData.Caller` и `EventData.CallerFunction` отдают позицию и имя функции
хукам форматирования.
...
Кроме именованного набора уровню можно задать цвет из палитры на 256 цветов или
24-битный:
...
`DepthAuto` читает `COLORTERM` и `TERM`, и всё, чего терминал не понимает,
понижается, а не печатается escape-кодами: 24-битный цвет становится ближайшим
из палитры на 256-цветном терминале и ближайшим базовым на 16-цветном.
`Depth16`, `Depth256` и `DepthTrueColor` задают глубину принудительно.

`SetLevelColor` по-прежнему принимает `TextStyle` и остаётся коротким путём для
именованных цветов.
